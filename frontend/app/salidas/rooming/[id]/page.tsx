...
export default function RoomingPage() {
  ...
  useEffect(() => {
    const fetchReservas = async () => {
      ...
      try {
        const [data, hotelsParams, destinosParams, salidaData] =
          await Promise.all([
        ...
        if (Array.isArray(destinosParams)) {
          setDestinationNames(
            Object.fromEntries(
              destinosParams
                .filter((destination: any) => destination.id)
                .map((destination: any) => [
                  destination.id,
                  destination.name || destination.nombre || "Destino",
                ]),
            ),
          );
        }
            ...
            <section
              ...
              <div className="bg-primary text-white text-center py-1.5 rounded-b-xl px-4">
                <p className="text-xs font-semibold capitalize">
                  ...
                  {room.id.startsWith("sin-asignar-") ? "S/A" : room.id})
                </p>
              </div>
            </section>
          ))}
  ...
  const renderHotelRoomingSection = (
    resList: any[],
    hotelTitle?: string,
    hotelId?: string,
  ) => {
    ...
    resList.forEach((r) => {
      ...
      const paxs =
        hotelId === undefined
          ? allPaxs
          : allPaxs.filter(
              (pax: any) => (pax.hotel_id || r.hotel_id || "") === hotelId,
            );
      ...
      const roomsToCreate =
        roomDetailsList.length > 0
          ? roomDetailsList
          : [parseRoomItem("doble_matrimonial_estandar")];

      const hasRoomIndexes = paxs.some(
        (pax: any) => pax.room_index !== undefined && pax.room_index !== null,
      );
      if (hasRoomIndexes) {
        const passengersByRoom = new Map<number, any[]>();
        paxs.forEach((pax: any) => {
          const roomIndex = Number(pax.room_index) || 0;
          passengersByRoom.set(roomIndex, [
            ...(passengersByRoom.get(roomIndex) || []),
            pax,
          ]);
        });
        Array.from(passengersByRoom.entries())
          .sort(([left], [right]) => left - right)
          .forEach(([roomIndex, roomPaxs]) => {
            const roomDetail = roomsToCreate[roomIndex] || roomsToCreate[0];
            const roomIdLabel = r.rooming_id
              ? roomsToCreate.length > 1
                ? `${r.rooming_id} (${roomIndex + 1})`
                : r.rooming_id
    ...
    const dobleIndividualRooms = roomsList.filter((r) => {
      ...
      return (
        parsed.camaCode === "DBL" &&
        (parsed.distribucionCode === "IND" || parsed.distribucionCode === "TWN")
      );
    });
    ...
    const otherRooms = roomsList.filter((r) => {
      const parsed = parseRoomItem(r.type);
      const isMatched =
        (parsed.camaCode === "DBL" && parsed.distribucionCode === "MAT") ||
        (parsed.camaCode === "DBL" &&
          (parsed.distribucionCode === "IND" ||
            parsed.distribucionCode === "TWN")) ||
        parsed.camaCode === "SGL" ||
        parsed.camaCode === "TPL" ||
        parsed.camaCode === "CPL";
      return !isMatched;
  ...
  const handleExportHotel = async (
    ...
    try {
      const hotelReservation = activeReservations.find((reservation) => {
        const passengers = reservation.reservation_passengers?.length
          ? reservation.reservation_passengers
          : [reservation];
        return passengers.some(
          (passenger: any) =>
            (passenger.hotel_id || reservation.hotel_id || "") === hotel.id,
        );
      });
      await exportRoomingHotelToExcel({
        hotel,
        destino:
          destinationNames[salida?.destino] || salida?.destino || "Destino",
        checkIn: salida?.date_of_out || "-",
        checkOut: salida?.date_of_return || salida?.fecha_regreso || "-",
        regimen: hotelReservation?.regimen_nombre || "-",
      });
      toast.success(`Rooming de ${hotel.name} descargado`);
    } catch (error) {
      console.error("Error al exportar rooming:", error);
      toast.error("No se pudo generar el Excel de rooming");
    } finally {
  ...
  const packageGroups = Array.from(
    new Set(
      activeReservations.map(
        (reservation) => reservation.package_id || "__without_package__",
      ),
    ),
  ).map((packageKey) => {
    ...
    const usedHotelIds = Array.from(
      new Set(
        packageReservations.flatMap((reservation) => {
          const passengers = reservation.reservation_passengers?.length
            ? reservation.reservation_passengers
            : [reservation];
    ...
    return {
      key: packageKey,
      title:
        packageInfo?.name_system ||
        packageInfo?.name ||
        (packageKey === "__without_package__"
          ? "Reservas sin paquete"
          : "Paquete"),
      reservations: packageReservations,
      hotels: usedHotelIds.map((hotelId) => {
        const packageHotel = packageInfo?.hotels?.find(
          (hotel: any) => hotel.hotel_id === hotelId,
        );
        const hotelName =
          hotelNames[hotelId] || (hotelId ? "Hotel" : "Sin hotel asignado");
        return {
          id: hotelId,
    ...
    <Container>
      ...
      <section className="flex flex-col gap-3 mb-6">
        <Link
          ...
          <ArrowLeft />
          <h1 className="font-bold text-primary text-sm md:text-base">
            Volver al menú
          </h1>
        </Link>
        <button
          ...
          <h1 className="font-semibold text-secondary text-sm md:text-base">
            Volver a Salidas
          </h1>
        </button>
      </section>
        ...
        <section className="flex flex-wrap gap-2 mb-6">
          {roomingHotels.map((hotel) => (
            <button
              ...
              type="button"
              onClick={() => handleExportHotel(hotel)}
              disabled={exportingHotelId === hotel.id}
              className="text-black rounded-lg flex items-center gap-2 px-3 py-2 text-sm font-semibold disabled:opacity-60">
              <Excel />
              <span>
                {" "}
                {exportingHotelId === hotel.id
                  ? "Generando Excel..."
                  : `Exportar Excel ${hotel.name}`}
              </span>
            </button>
          ))}
        </section>
