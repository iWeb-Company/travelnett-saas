...
export default function ReservaIdPage() {
  ...
  const [lugaresCarga, setLugaresCarga] = useState<
  ...
  const [clientCommissionPct, setClientCommissionPct] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .then((data) => {
        ...
        if (
          data.reservation_passengers &&
          Array.isArray(data.reservation_passengers)
        ) {
          const allIndexesZero = data.reservation_passengers.every(
            (rp: any) => !rp.room_index || rp.room_index === 0,
          );
          ...
          const mapped = data.reservation_passengers.map(
            (rp: any, i: number) => {
              const rawName = rp.nombre_completo || "";
              const parts = rawName.trim().split(" ");
              const first =
                rp.nombre || (parts.length > 1 ? parts[0] : rawName);
              const last =
                rp.apellido ||
              ...
              if (
                ...
                assignedRoom === null ||
                (allIndexesZero && parsedRooms.length > 1)
              ) {
              ...
              return {
                ...
                room_index:
                  assignedRoom !== undefined && assignedRoom !== null
                    ? assignedRoom
                    : 0,
              };
            },
    ...
    // Load Liquidacion
    apiClient
      .getLiquidacionByBooking(id)
      .then((liq) => {
        if (liq) {
          setLiquidacionId(liq.id);
          if (liq.total_amout !== null && liq.total_amout !== undefined)
            setTotalReserva(Number(liq.total_amout));
          const tc =
            liq.total_commission !== undefined && liq.total_commission !== null
              ? liq.total_commission
              : (liq.total_comisionable ?? liq.total_commissionable);
          if (tc !== null && tc !== undefined) setTotalComisionable(Number(tc));
          if (liq.commission !== null && liq.commission !== undefined)
            setCommission(Number(liq.commission));
          if (liq.gastos && Array.isArray(liq.gastos)) {
            setGastos(
              liq.gastos.map((g: any) => ({
                id: g.id,
                name: g.name,
    ...
    apiClient
      .getPagosReserva(user.iweb_client_id, id)
      .then((pagos) => {
  ...
  const handleAddRoomSubmit = () => {
    ...
    const distKey =
      camaKey === "single"
        ? "individual"
        : distMap[newRoomDistribucion] || "matrimonial";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (reserva.salida_id) {
        try {
          const salidaInfo = await apiClient.getSalida(
            user.iweb_client_id,
            reserva.salida_id,
          );
          if (salidaInfo) {
            ...
            const prevCama = (reserva.reservation_passengers || []).filter(
              (p: any) => (p.butaca_type || "").toLowerCase() === "cama",
            ).length;
            const prevSemicama = (reserva.reservation_passengers || []).filter(
              (p: any) => (p.butaca_type || "").toLowerCase() === "semicama",
            ).length;

            const availableCama =
              (salidaInfo.cama_disponibles !== undefined
                ? Number(salidaInfo.cama_disponibles)
                : 0) + prevCama;
            const availableSemicama =
              (salidaInfo.semicama_disponibles !== undefined
                ? Number(salidaInfo.semicama_disponibles)
                : 0) + prevSemicama;

            if (camaReq > availableCama) {
              toast.error(
                `No hay disponibilidad suficiente de butacas CAMA en la salida. Disponibles: ${availableCama}`,
              );
              setSaving(false);
              return;
            }
            if (semicamaReq > availableSemicama) {
              toast.error(
                `No hay disponibilidad suficiente de butacas SEMICAMA en la salida. Disponibles: ${availableSemicama}`,
              );
              setSaving(false);
              return;
            }
      ...
      const passengersPayload = paxSource
        ...
        .map((rp: any) => ({
          ...
          room_index:
            rp.room_index !== undefined && rp.room_index !== null
              ? rp.room_index
              : 0,
        }));
      ...
      try {
        ...
        if (updatedLiq) {
          ...
          if (
            updatedLiq.total_amout !== null &&
            updatedLiq.total_amout !== undefined
          )
          ...
          if (tc !== null && tc !== undefined) setTotalComisionable(Number(tc));
          if (
            updatedLiq.commission !== null &&
            updatedLiq.commission !== undefined
          )
            setCommission(Number(updatedLiq.commission));
          if (updatedLiq.gastos && Array.isArray(updatedLiq.gastos)) {
            setGastos(
              updatedLiq.gastos.map((g: any) => ({
                id: g.id,
                name: g.name,
                amount: Number(g.amount),
              })),
  ...
  const handleOpenSetRoomTypeModal = (idx: number) => {
    ...
    setModalDistribucionValue(
      detail.distribucionCode === "MAT" ? "matrimonial" : "twin",
    );
  ...
  const handleSubmitRoomType = () => {
    if (targetRoomIdx !== null) {
      ...
      const distKey =
        camaKey === "single"
          ? "individual"
          : distMap[modalDistribucionValue] || "matrimonial";
  ...
  const handleCamaDistribucionChange = (idx: number, newCamaDist: string) => {
    ...
    const distKey =
      detail.distribucionCode === "MAT" ? "matrimonial" : "individual";
    const tipoKey = getTipoHabitacionKey(detail);
  ...
  const handleTipoHabitacionChange = (idx: number, newTipoHab: string) => {
    ...
    const distKey =
      detail.distribucionCode === "MAT" ? "matrimonial" : "individual";
    const tipoKey =
      newTipoHab.toLowerCase().includes("superior") || newTipoHab === "SUP"
        ? "superior"
        : newTipoHab.toLowerCase().includes("suite") || newTipoHab === "SUI"
          ? "suite"
          : "estandar";
  ...
  const getPassengersForRoom = (roomIdx: number) => {
    ...
    if (isUnpartitioned) {
      ...
      roomPaxs = source
        .slice(startIndex, startIndex + cap)
        .map((p: any, offset: number) => ({
          ...p,
          ...
          {gastos.map((gasto, index) => (
            <div
              ...
              <input
                ...
                value={gasto.name}
                onChange={(e) =>
                  handleGastoChange(index, "name", e.target.value)
                }
        ...
        <ModalLayout
          ...
          <section className="flex flex-col gap-5">
            ...
            <p className="text-white text-center font-bold text-base">
              ...
              {targetRoomIdx !== null ? targetRoomIdx + 1 : ""}
            </p>
                ...
                <tr className="py-2 h-10">
                  ...
                  <th>
                    <select
                      ...
                      onChange={(e) =>
                        setModalDistribucionValue(e.target.value)
                      }
                      className="bg-gray-600 border border-gray-500 rounded-lg p-1.5 w-full text-center cursor-pointer text-white font-medium">
