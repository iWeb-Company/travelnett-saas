...
export default function CuentasPage() {
  ...
  const handleCreate = async () => {
    ...
    formData.append("account_title", cuentasData.account_title);
    formData.append("account_number", cuentasData.account_number);
    formData.append("titular", cuentasData.titular);
    formData.append("cuit_cuil", cuentasData.cuit_cuil);
    formData.append("cbu_cvu", cuentasData.cbu_cvu);
    formData.append("alias", cuentasData.alias);
        ...
        <ModalLayout
          ...
          <form className="flex flex-col gap-4" onSubmit={(e) => e.preventDefault()}>
            ...
            <input
              ...
              value={cuentasData.cuit_cuil}
              onChange={(e) =>
                setCuentasData({ ...cuentasData, cuit_cuil: e.target.value })
              }
              className="w-full shadow-lg shadow-black/30 bg-white rounded-sm p-2 pr-4 text-black/90 font-medium focus:outline-none"
            />
            <input
              type="text"
              placeholder="CBU/CVU"
              value={cuentasData.cbu_cvu}
              onChange={(e) =>
                setCuentasData({ ...cuentasData, cbu_cvu: e.target.value })
              }
              className="w-full shadow-lg shadow-black/30 bg-white rounded-sm p-2 pr-4 text-black/90 font-medium focus:outline-none"
            />
            <input
